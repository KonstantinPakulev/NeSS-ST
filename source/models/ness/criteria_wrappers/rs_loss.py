...
class RSLoss(CriterionWrapper):

    def __init__(self, model_mode_wrapper, config):
        self.ners_detector = deepcopy(model_mode_wrapper.model_wrapper.modules_wrappers[0].detector)

        self.scale_factor = config.scale_factor
        self.num_samples = config.num_samples

        self.config = OmegaConf.merge(*[config, config.get(self.ners_detector.base_detector.get_name(), {})])

        self.patch_size = self.config.nms_size + self.ners_detector.base_detector.get_border_size() * 2
        self.scaled_patch_size = int(self.patch_size * self.scale_factor) // 2 * 2 + 1

        self.ners_detector.base_detector.set_border_size(self.scaled_patch_size + int(4 * self.scale_factor))
        self.ners_detector.base_detector.set_loc(False)

    def forward(self, engine, device, batch, bundle, endpoint):
        image_gray = batch[du.IMAGE_GRAY1].to(device)
        ners = bundle[n_ns.NERS1].to(device)

        kp, kp_score, kp_base_score = self.ners_detector(image_gray, ners.detach(), self.config)
        ...
        kp_rs = get_repeatability_score(image_gray, kp, h_data,
                                        ...
                                        self.ners_detector.base_detector)

        kp_ners = sample_tensor(ners, kp, ners.shape).squeeze(-1)

        loss = mse_loss(kp_ners, kp_rs, kp_mask)
