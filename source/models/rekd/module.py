...
class REKD(torch.nn.Module):
    ...
    def from_config(config):
        return REKD(config.factor_scaling_pyramid,
                    config.group_size,
                    config.dim_first, config.dim_second, config.dim_third)

    def __init__(self, factor_scaling_pyramid, group_size,
                 dim_first, dim_second, dim_third):
        super(REKD, self).__init__()

        self.pyramid_levels = 3
        self.factor_scaling = factor_scaling_pyramid
        ...
        self.feat_type_in = nn.FieldType(r2_act,
                                         num_channels * [r2_act.trivial_repr])  ## input 1 channels (gray scale image)

        feat_type_out1 = nn.FieldType(r2_act, dim_first * [r2_act.regular_repr])
        feat_type_out2 = nn.FieldType(r2_act, dim_second * [r2_act.regular_repr])
        feat_type_out3 = nn.FieldType(r2_act, dim_third * [r2_act.regular_repr])
        ...
        self.last_layer_learner = torch.nn.Sequential(
            torch.nn.BatchNorm2d(num_features=dim_third * self.pyramid_levels),
            torch.nn.Conv2d(in_channels=dim_third * self.pyramid_levels, out_channels=1, kernel_size=1, bias=True),
            torch.nn.ReLU(inplace=True)  ## clamp to make the scores positive values.
        )

        self.dim_third = dim_third
